...
namespace WesServiceAW2014
{
    ...
    public class WebService1 : System.Web.Services.WebService
    {
        ...
        [WebMethod]
        public List<ProductView> getProductCulture(string culture)
        {
            List<ProductView> lstP = new List<ProductView>();

            DataTable dtP = DataHelper.getProductView(culture);

            foreach(DataRow dr in dtP.Rows)
            {
                ProductView pv = new ProductView();
                pv.CultureID = dr["CultureID"].ToString();
                pv.Description = dr["Description"].ToString();
                pv.ProductID = dr["ProductID"].ToString();
                pv.Name = dr["Name"].ToString();
                pv.ProductModel = dr["ProductModel"].ToString();
